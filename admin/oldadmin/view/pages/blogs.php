		...
		<form>
		  <div class='col-sm-11 row'>	  
			...
			<div class='col-sm-1 row'>
			  <button class='btn btn-success'><a class="text-white"href='?page=news'>დამატება</a></div></button>
			</div>
		</div>
	...
	?>
		  <tr>
	       ...
	       for($z=0;$z<count($lnarr);$z++)
			...
			<th><a href="?page=lesson&id=<?=$r1["id"]?>"><button class="btn btn-outline-success">რედაქტირება</button> </a> </th>
...
<?php
$q3=mysqli_query($con,"SELECT * FROM articles as t1 WHERE t1.id>0 AND t1.news=1 $WSE ");
$toto=0;
if($q3){
...
?>
	...
	<a href="?page=newses&p=1<?=$ksarch?>" class="PG USR">«</a>
	<a href="?page=newses&p=<?=$ACP!=1?($ACP-1):$ACP?><?=$ksarch ?>" class="PG USR">‹</a>
	<?php
	for($i=1;$i<=ceil($toto/$PA);$i++){
		if(($ACP-5)<=$i&&($ACP+5)>=$i){
	?>
	<a href="?page=newses&p=<?=$i?><?=$ksarch ?>" class="PG <?=($ACP==$i?"ACP":"")?> USR"><?=$i?></a>
	<?php }
	} 
	?>
	<a href="?page=newses&p=<?=$ACP!=ceil(mysqli_num_rows($q3)/$PA)?($ACP+1):$ACP?><?=$ksarch ?>" class="PG USR">›</a>
	<a href="?page=newses&p=<?=ceil(mysqli_num_rows($q3)/$PA);?><?=$ksarch ?>" class="PG USR">» <?=ceil(mysqli_num_rows($q3)/$PA);?></a>
	</div>
</div>
</div>
<br>