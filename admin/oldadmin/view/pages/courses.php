...
<div class="col-md-12 p-0 H mt-2 my-2">
	...
	<div class="col-md-12 LIS H bgbox itmcontainer" t="categories" norep="name" conf='' message='კურსი უკვე არსებობს!' d='' slug='1' n='1' >  
	...
	$c=0;
	   ...
	   $c++;
	?>
		<div class="D2" style="width:280px;"><input placeholder="კურსის დასახელება <?=$lnshortarr[$z]?>" class="UPTS form-control"   tp='' ln='<?=$lnshortarr[$z]?>' name='name'/></div>
		<div class="D2" style="width:280px; display:none"><input placeholder="კურსის დასახელება" class="UPTS form-control"   tp='' ln='<?=$lnshortarr[$z]?>' name='description'/></div>
		<div class="D2" style="width:280px; display:none"><input placeholder="კურსის დასახელება" class="UPTS form-control"   tp='' ln='<?=$lnshortarr[$z]?>' name='keywords'/></div>
	...
	<?php
	   ...
	   ?>	
 <div class="D2" style="width:280px; display:none"><input placeholder="კურსის დასახელება" class="UPTS form-control"  name='type' ln='' tp='int' value="2" /></div>
...
?>
	...
	<a href="?page=courses&p=1&category=<?=$category?>&type=<?=$type ?>&active=<?=$active ?>" class="PG USR">«</a>
	<a href="?page=courses&p=<?=$ACP!=1?($ACP-1):$ACP?>&category=<?=$category?>&type=<?=$type ?>&active=<?=$active?>" class="PG USR">‹</a>
	<?php
	for($i=1;$i<=ceil(mysqli_num_rows($q3)/$PA);$i++){
		if(($ACP-5)<=$i&&($ACP+5)>=$i){
	?>
	<a href="?page=courses&p=<?=$i?>&category=<?=$category?>&type=<?=$type ?>&active=<?=$active?>" class="PG <?=($ACP==$i?"ACP":"")?> USR"><?=$i?></a>
	<?php }
	}
	?>
	<a href="?page=courses&p=<?=$ACP!=ceil(mysqli_num_rows($q3)/$PA)?($ACP+1):$ACP?>&category=<?=$category?>&type=<?=$type ?>&active=<?=$active?>" class="PG USR">›</a>
	<a href="?page=courses&p=<?=ceil(mysqli_num_rows($q3)/$PA);?>&category=<?=$category?>&type=<?=$type ?>&active=<?=$active?>" class="PG USR">» <?=ceil(mysqli_num_rows($q3)/$PA);?></a>
	</div>
</div>
</div>