...
if($a!=""){
	$q1=mysqli_query($con,"SELECT * FROM library WHERE id='".$a."'");
	$r1=mysqli_fetch_array($q1);
}else{

	$ina=mysqli_query($con,"INSERT INTO library (date) VALUES ('$T') ");

	$dd=mysqli_query($con, "SELECT id FROM library  ORDER BY id DESC");
...
<script>location.href="?page=book&id=<?=$a?>"</script>
<?php
	  ...
	  for($z=0;$z<count($lnarr);$z++)
	   {
	   $c++;
 $lng=mysqli_query($con, "SELECT id FROM langs WHERE shortname='".$lnshortarr[$z]."' AND tableName='library' AND  tableColumn='title' AND tableId='$a' ");
 $lng1=mysqli_query($con, "SELECT id FROM langs WHERE shortname='".$lnshortarr[$z]."' AND tableName='library' AND  tableColumn='text' AND tableId='$a' ");
 $lng2=mysqli_query($con, "SELECT id FROM langs WHERE shortname='".$lnshortarr[$z]."' AND tableName='library' AND  tableColumn='description' AND tableId='$a' ");
 $lng3=mysqli_query($con, "SELECT id FROM langs WHERE shortname='".$lnshortarr[$z]."' AND tableName='library' AND  tableColumn='keywords' AND tableId='$a' ");
 $lng4=mysqli_query($con, "SELECT id FROM langs WHERE shortname='".$lnshortarr[$z]."' AND tableName='library' AND  tableColumn='shorttext' AND tableId='$a' ");
   if(mysqli_num_rows($lng)<1)
   {
	   
	  
	   mysqli_query($con,"INSERT INTO langs SET shortname='".$lnshortarr[$z]."' , tableName='library' , tableColumn='title' , tableId='$a'  ");
   }
    ...
    if(mysqli_num_rows($lng1)<1)
   {
	   ...
	   mysqli_query($con,"INSERT INTO langs SET shortname='".$lnshortarr[$z]."' , tableName='library' , tableColumn='text' , tableId='$a'  ");
   }
    ...
    if(mysqli_num_rows($lng2)<1)
   {
	   ...
	   mysqli_query($con,"INSERT INTO langs SET shortname='".$lnshortarr[$z]."' , tableName='library' , tableColumn='description' , tableId='$a'  ");
   }
    if(mysqli_num_rows($lng3)<1)
   {
	   
	  
	   mysqli_query($con,"INSERT INTO langs SET shortname='".$lnshortarr[$z]."' , tableName='library' , tableColumn='keywords' , tableId='$a'  ");
   }
     if(mysqli_num_rows($lng4)<1)
   {
	   
	  
	   mysqli_query($con,"INSERT INTO langs SET shortname='".$lnshortarr[$z]."' , tableName='library' , tableColumn='shorttext' , tableId='$a'  ");
   }
	   }
...
<?php
 ...
 $a=mysqli_real_escape_string($con,$_GET["id"]??"");
 $q1=mysqli_query($con,"SELECT t1.*,  ". languages('library','t1.id','title') ." ,
                                      ".languages('library','t1.id','text') .",
                                      ".languages('library','t1.id','description') .",
                                      ".languages('library','t1.id','shorttext') .",
                                      ".languages('library','t1.id','keywords') ."
									  FROM library  AS t1 WHERE t1.id='".$a."'");
 $r1=mysqli_fetch_array($q1);
?>
<div role="tabpanel">
    <!-- Tab panes -->
    ...
    <div class="tab-content  itmcontainer" t='library'>
...
<input class="form-control UPTS d-none" value="<?=$r1["slug"] ?>" rows="10" data-locale="ge" tp='' ln='' id="SLGGE" name="slug" cols="50" placeholder='slug' />
        ...
        <div class="col-sm-12 pad-no-right pad-no-xs my-2">
            <label for="pdf">ფასი</label> <small>800px/800px</small>

<div class="input-append row">
					<div class="col-sm-12 pad-no-right pad-no-xs my-2">
						<input class="form-control UPTS"  tp='double' ln=''  placeholder="ფასი" type="number" name='price' value="<?=$r1["price"]?>">		
...
<div class="input-append row">
					<div class="col-md-9">
						<input id="YDA9767032" class="form-control UPT" t="library" n="img" d="<?=$a ?>" placeholder="სურათი 1" type="text" value="<?=$r1["img"]?>">		
					</div>
  ...
  <div class="row nonTranslatedInput pad-no-sm">
        ...
        <div class="col-sm-12 pad-no-right pad-no-xs my-2">
            <label for="pdf">წიგნი</label> 

<div class="input-append row">
					<div class="col-md-9">
						<input id="YDA97670321" class="form-control UPTS"  name="book" tp='' ln='' placeholder="წიგნი" type="text" value="<?=$r1["book"]?>">		
					</div>
					&nbsp;<a href="javascript:open_popup('responsive_filemanager/filemanager/dialog.php?type=2&popup=1&field_id=YDA97670321&relative_url=0')"><button class="btn iframe-btn btn-outline-success">არჩევა</button></a>
				</div>
...
<?php
$q2=mysqli_query($con,"SELECT t1.*, ". languages('categories','t1.id','name') ." FROM categories AS t1 WHERE t1.type IN(SELECT id FROM ctypes WHERE name='library') AND t1.active='1' ORDER BY t1.id DESC");
...
?>				
    ...
    <div class="submit my-4">
        <button  class="btn btn-success ADDITEMS" slug='1' t='library' d="<?=$a?>">დამახსოვრება</button>
    </div>
