<?php
	 ...
	 function permissions($x)	{
		...
		//$usrs=mysqli_query($con,"SELECT * FROM admins WHERE id='$uid' ");
		...
		$permissions=mysqli_query($con,"SELECT t1.*, 
											   ...
											   (SELECT  custom  FROM admins WHERE id=$uid) AS custom,
											  
											   (SELECT (SELECT typeName FROM userTypes WHERE id=t2.type)   FROM admins AS t2 WHERE t2.id=$uid) AS typeName
											    FROM permissionMeta AS t1
											   WHERE (t1.uid=(SELECT id FROM admins WHERE custom=1 AND id=$uid)  ) OR (t1.roleid =(SELECT type FROM admins WHERE custom!=1 AND id=$uid) ) ORDER BY t1.id ASC")	;															  
