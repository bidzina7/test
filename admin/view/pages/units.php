<?php
 ...
 $wsearch=$search!=""?" AND t1.id IN(SELECT tableId FROM langs WHERE tableColumn='name' AND tableName='units' AND columnValue LIKE '%$search%')":"";  
?>
			...
			<div class="card-header">
				<div class="w-100">
					<div class="btn btn-outline-success px-3 cslitms" d='1'>
						გაზომვის ერთეულები <i class="fal fa-plus ml-2"></i> 
					</div>
				</div>
			</div>
			 ...
			 <div class="card  tab-content  mt-3 slitms" d='1'>
					...
					<div class="card-body a itmcontainer" t='units' n='1'  >
	                    ...
	                    for($z=0;$z<count($lnarr);$z++)
	                    {
	                     ...
	                     ?>
							  <div class="mb-2">
								<span class="border-0 ">გაზომვის ერთეულის დასახელება <?=$lnshortarr[$z] ?></span>
						        <input type="text" class="form-control mt-2 UPTS" name='name' tp="" ln='<?=$lnshortarr[$z] ?>' placeholder="გაზომვის ერთეულის დასახელება">
							  </div>
						<?php
						}
                        ?>						
                     ...
                     <div class="col-12 mt-3 px-0">
						 <div class="btn btn-outline-success ADDITEMS" msg="შენახულია"   t='units'  wr='1' pagename='' n='1'  d="" pos='1'>
							 დამატება
						 </div>
					 </div>
					 </div>
                    ...
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-striped">
                                <thead class="bg-light">
                                    <tr class="border-0">
	                                    ...
	                                    for($z=0;$z<count($lnarr);$z++)
	                                    {
	                                      $c++;
	                                   ?>
                                        <th class="border-0">გაზომვის ერთეული <?=$lnshortarr[$z] ?></th>
									   <?php
										}
                                        ?>										
                             ...
                             $start=($pg-1)*$lmt;											
					    $fld=mysqli_query($con,"SELECT t1.*,  ". languages('units','t1.id','name') ."  FROM units AS t1 WHERE t1.id!=0  $wsearch ORDER BY t1.position ASC LIMIT $start, $lmt ");
				      $i=$start;
                                    ...
                                    <tr class=''>
	                                   ...
	                                   ?>
                                        <td>
										 ...
										 <input type='text' class="form-control UPT" rl='1' value='<?=$rfld["name".$lnshortarr[$z]] ?>' t='units' n='name' ln='<?=$lnshortarr[$z] ?>' d='<?=$rfld['id'] ?>' />   
                                        ...
                                        <td>
                                            <button class="btn  btn-danger del " t='units'  d="<?=$rfld["id"] ?>	" ><i class="fa fa-trash text-white"></i></button>
                                        </td>
                                    </tr>
									<?php
						            }
               ...
               <?php
			      $pgs=mysqli_query($con,"SELECT t1.id FROM units AS t1 WHERE t1.id!=0 $wsearch");
