...
// echo "SELECT t1.*, SELECT DISTINCT((columnValue) AS a FROM langs WHERE tableId=t1.id AND tableColumn='companyname' AND tableName='protocol' AND shortname='ka') AS disname 	
                                 // FROM protocol AS t1";
        $usr=mysqli_query($con, "SELECT DISTINCT(columnValue) AS disname FROM langs WHERE  tableColumn='comment' AND tableName='protocol' AND shortname='ka' AND columnValue LIKE '%$vl%'") ;
	...
	<?php
			 // $usr1=$usr=mysqli_query($con, "SELECT t1.*,   
											  // " . languages('protocol','t1.id','comment') ."  FROM protocol AS t1 WHERE t1.id IN(SELECT tableId FROM langs WHERE tableName='protocol' AND tableColumn='comment' AND columnValue='$vl')");
			 // $rusr1=mysqli_fetch_assoc($usr1);
 // echo $rusr1['commentka'] ." ---".$rusr1['commenten'] ." ---".$rusr1['commentru'] ;
	?>